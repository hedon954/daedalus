...
- Current gap: `react.rs` 仍直接调用 `tool::function::run_tool`；下一步是让 `react.rs` 改为调用 `ToolRuntime`。
...
这里不应该引入 `CommandRequest`。`add/sub` 可以进入统一 runtime，但它们进入的是 pure function path。
...
当前 `tool/runtime.rs` 只有空骨架。建议下一步按这个方向填。
