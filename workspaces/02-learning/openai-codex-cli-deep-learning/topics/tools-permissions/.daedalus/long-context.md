...
- 2026-06-07 Slice 9 第一版已实现：`react.rs` 使用 `tokio::spawn + join_all` 并发执行同批 tool calls，并按原始 index 回灌 observations；已补 mixed success / failure / denial 测试，`cargo test` 为 64 passed、3 ignored。下一步按 `guides/08-demo-coder/12-slice-9-tool-batch-runner-refactor.md` 抽出 `ToolBatchRunner / ToolRunOutcome / ToolObservation`，降低 ReAct loop 职责密度。
...
- `demo/design.md` 已从 draft 收敛为 architecture blueprint，核心数据结构、状态机、事件协议和验收用例已有设计；当前 `08-demo-coder` 已完成 Slice 9 multi-tool independent execution 第一版，下一步收口 ToolBatchRunner 结构。
...
- 当前导航：不是继续泛读 Codex 权限系统；`auth/approval/sandbox` 已收敛成 demo 不变量。当前按 `guides/08-demo-coder/README.md` 的 slice 地图编码；下一步围绕 Slice 9 抽出 ToolBatchRunner，避免继续把 batch 调度堆在 ReAct loop。
