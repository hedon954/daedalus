...
- 2026-06-05 Slice 8 Event Protocol Hardening 进行中：事件类型包含 `CommandNeedsApproval`、`CommandExecutionStarted/Finished/Failed`、`CommandRetryEvaluated`；已从旧 `ApprovalController / ApprovalBroker` 重构为 `ApprovalGateway + PendingApproval`，由 `run_shell_command` 通过当前 run 的 `EventSender` 发送 `CommandNeedsApproval`，内部 `ToolApprovalResult` 通过 `approval_id + oneshot` 回流；send failure fail closed、pending cleanup、retry 成功路径 attempt trace 和命名残留均已修复，`cargo test` 为 63 passed、3 ignored。下一步按 `guides/08-demo-coder/09-slice-8-command-event-emitter-refactor.md` 评估/引入 `CommandEventEmitter` 和 `run_execution_attempt`，收敛散落的 command event 发送。
...
- `demo/design.md` 已从 draft 收敛为 architecture blueprint，核心数据结构、状态机、事件协议和验收用例已有设计；当前 `08-demo-coder` 正在推进 Slice 8 Event Protocol Hardening，approval request 的 run-scoped event stream 已闭合，下一步是评估是否统一 command event emitter。
...
- 当前导航：不是继续泛读 Codex 权限系统；`auth/approval/sandbox` 已收敛成 demo 不变量。当前按 `guides/08-demo-coder/README.md` 的 slice 地图编码；下一步是围绕 Slice 8 判断 `CommandEventEmitter / run_execution_attempt` 是否值得引入，以收敛已跑通但仍散落的 command event 发送。
