...
- 当前问题：Slice 8 Event Protocol Hardening 进行中：`ApprovalBroker/ApprovalController` 已重构为 `ApprovalGateway + PendingApproval`，approval request 由当前 `run_shell_command` 的 `EventSender` 发到当前 run stream，send failure 会 fail closed 并清理 pending；retry success trace tests 已显式锁定 `SandboxFirst -> Failed -> RetryEvaluated -> NoSandboxRetry` 顺序。当前剩余结构风险是 command event 发送仍散落在业务分支中。
...
- Current open decision：是否继续把 command-level event 收敛到 `CommandEventEmitter` / `run_execution_attempt`。`CommandNeedsApproval` 已经通过当前 run 的 `EventSender` 透出，不再需要 `ApprovalController` 绑定事件。
...
- [ ] Slice 8 Event Protocol Hardening：`ApprovalGateway + PendingApproval`、run-scoped `CommandNeedsApproval`、command attempt / retry event 和 trace tests 已完成；待决定是否用 `CommandEventEmitter` / `run_execution_attempt` 收敛事件出口，降低散落 `tx.send` 的维护风险。
