...
| Trainer / full loop | Trainer 如何封装 dataloader、forward、loss、backward、optimizer、scheduler 与 evaluation？手写循环又暴露什么？ | 已生成 [`17-chapter3-finetuning-pretrained-model.md`](../topics/lora-feedback-loop/guides/04-lesson-lab/17-chapter3-finetuning-pretrained-model.md)，全章机制已预读 | 需要用户从 3/2 开始补齐数据处理、optimizer/scheduler、evaluation 与 Accelerate 的可观察证据 | 当前 chapter 主线 |
