...
| LLM inference | LLM 如何从 prompt 逐 token 生成？推理成本、延迟和显存主要花在哪里？ | Chapter 1/8 `Deep dive into Text Generation Inference with LLMs` 已进入；官方主线包括 context/attention、prefill/decode、sampling controls、TTFT/TPOT/throughput/VRAM、KV cache | 需要用户理解 prefill 与 decode 的分工，以及 KV cache 为什么能减少重复计算但占用显存 | 当前 lesson |
