...
| sequence classification | 文本分类任务如何从 `text + label` 反推到 tokenizer、collator、model head、metric 和 Trainer？ | 用户已完成 `sequence_classification.ipynb`；已观察 batch keys、loss、logits shape、argmax、id2label；已生成 [`04-sequence-classification-derivation.md`](../topics/lora-feedback-loop/guides/04-lesson-lab/04-sequence-classification-derivation.md) | 仍需闭卷重写最小 flow；需确认 `id2label` 与数据集 label 语义一致 | 当前 review |
