...
第一版训练输出不要过重。小模型容易在复杂 JSON 上失稳，因此训练目标采用最小结构：

```json
{
  "suggestions": [
    {
      "suggestion": "让卖家补一张屏幕纯白背景照片和边框四角近拍。",
      "reason": "当前缺少成色证据，屏幕和边框瑕疵会直接影响真实价值和议价空间。"
    }
  ]
}
```
...
- `suggestion` 适合作为最终产品展示。
- `reason` 方便训练、评测和用户审核。

复杂信息不让模型第一阶段生成，而是放在标注和评测侧：
...
- 小模型只学习“给出下一步建议 + 解释原因”，难度更低。
- eval 仍然可以用 `stage_fit / risk_tags / priority` 判断输出是否贴合阶段、覆盖关键风险、优先级是否合理。
- 后续如果迁移到更强模型或百炼平台，可以把这些字段升级为模型输出。
- 避免 fine-tuning 主要学到格式补全，而不是 suggestion next action 的质量。

第一阶段不要退回纯字符串数组作为唯一标注格式，因为那会丢掉训练和评测所需的解释依据。推荐边界是：模型输出 `suggestion + reason`，标注/eval 元信息保持丰富。
