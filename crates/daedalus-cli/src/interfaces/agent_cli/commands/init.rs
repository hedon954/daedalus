...
use crate::application::init_task::{
    InitCourseLearningOptions, InitTaskOptions, init_course_learning, init_repo_learning,
};
...
pub enum InitKind {
    ...
    /// 初始化 course learning 任务。
    CourseLearning(InitCourseLearningArgs),
}
...
/// `init course-learning` 参数。
#[derive(Debug, Args)]
pub struct InitCourseLearningArgs {
    /// 学习任务名称。
    pub name: String,
    /// 初始专题 slug。
    #[arg(long)]
...
impl CmdExecutor for InitCourseLearningArgs {
    async fn execute(self, ctx: AgentCliContext) -> Result<()> {
        debug!(task = %self.name, course_url = %self.course_url, "初始化 course learning 任务");
        let output = init_course_learning(InitCourseLearningOptions {
            repo_root: ctx.repo_root,
            name: self.name,
            topic_slug: self.topic,
            topic_title: self.title,
