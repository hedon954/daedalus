...
daedalus-tui 还是需要再优化一下，它的功能应该是这样的：

1. 只支持在 daedalus 根目录及其任意子目录下运行。
2. 如果是在特定的学习目录下执行，则跟当前一样。
3. 如果是在其他目录下执行，则扫描 01/02/03/04 所有的学习任务，并支持选择，跳转到第 2 步展示的结果。
4. 01/02/03/04 的 UI 应该是有所不同的，侧重点不一样。

---

感觉 @crates/daedalus-cli/src/application/transition_stage.rs  没有充分利用好  StateTransition 这个 trait。还是使用了类似 switch case 的方式。

---

评估一下这么重构后，代码是更简洁更易懂更好维护了，还是“炫技”？

---

请把 2 种方案都写入 plan 里面并进行客观对比，不要附和我。

---

绘制一些 mermaid 和说明，以 repo learning 为例子，详细解释我们当前这个实现的 10 个 stage 和其中的 status 是如何流转起来并串成一个完整的学习闭环的？输出到 @crates/docs 目录中。
