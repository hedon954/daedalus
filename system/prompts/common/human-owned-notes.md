...
写入 notes 前必须先判断笔记形态，不要把所有内容都追加到阶段 `README.md`。
...
Closeout 应聚焦核心主线，但不能忽略重要底层原理缺口。用户不需要在 closeout 里复述所有额外通用知识；但应该点名学习过程中暴露的第一性原理、底层原理、Rust/工程技能、外部最佳实践和替代方案。Agent 在 knowledge extraction 阶段必须回看这些线索，并基于 `reflection/candidate-map.md` 查漏补缺、去重和降噪。
