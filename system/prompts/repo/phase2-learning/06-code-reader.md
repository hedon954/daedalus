...
5. 每轮代码阅读都要实时写入 `notes/code-reading.md` 或专题 notes，记录“生产问题、用户猜测、Agent 校准、源码证据、不变量、代价、验证状态”，避免 Agent 直接替用户完成理解。
...
- 用户回答、纠正或验证关键点后，必须先更新 `notes/`，再继续下一段源码阅读；不能只在聊天中总结。
