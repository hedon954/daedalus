...
- 用户准备从阅读进入实现。
- 需要把学习目标转成可验收 demo。
- active topic 的 `.daedalus/outcome-map.md` 中指向 demo 的 open gaps 已经收敛到可以做设计决策。
- active topic 的 `demo/design.md` 已有草案，且关键字段不再被核心源码证据阻塞。
...
- 有意识地模仿 repo 的核心机制，而不是默认重写成“更简单”的方案。
- 明确哪些复杂度来自 repo 的生产约束，哪些只是当前 demo 不需要的历史包袱。
- demo 必须能运行、能测试、能解释。

## Repo-Specific Workflow

1. 读取 active topic 的 `.daedalus/outcome-map.md` 和 `demo/design.md` 草案，确认 North Star、当前缺口和 stop rules。
2. 选择一个核心能力，不超过一个主链路。
3. 明确从原 repo 忠实模仿的架构决策，并把每个决策绑定到已验证源码证据或用户确认的学习目标。
4. 明确刻意简化、改进或丢弃的部分，避免无意识照搬。
5. 定义模块、数据结构、主链路、状态机和验收用例。
6. 写出不做清单，防止范围膨胀。
7. 更新 active topic 的 `.daedalus/outcome-map.md`：把 demo 设计相关 gap 标为已解决或仍待实现验证。
...
- 不要复刻完整 repo；但不要把关键机制过度简化到失去实现手感。
- 验收用例必须在编码前确定。
